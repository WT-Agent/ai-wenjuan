  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">调研目的</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">问卷类型</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">调查问卷设计与防无效数据通关指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 调查问卷设计与调研题目专家');
...
const metricsList = [
  {
    "key": "questionnaireLogicRigor",
    "label": "问卷逻辑严谨度"
  },
  {
    "key": "scaleDesignPrecision",
    "label": "量表设计精准度"
  },
  {
    "key": "antiBiasObjectivity",
    "label": "防偏见中立客观度"
  },
  {
    "key": "userCompletionEase",
    "label": "填答填报易用性"
  },
  {
    "key": "insightDataValue",
    "label": "调研数据洞察价值"
  }
];
const guideRules = [
  {
    "title": "控制完成时间在 3-5 分钟",
    "advice": "总题目控制在 10-15 题以内，填答时间超过 5 分钟流失率将上升 60%",
    "avoid": "切忌设计动辄 30 道以上冗长繁琐的问卷"
  },
  {
    "title": "题目中立避免诱导",
    "advice": "用客观中立措辞（如“您对该功能的使用感受是？”），选项覆盖“非常满意”到“非常不满意”",
    "avoid": "避免在题目中使用“您是否也觉得该功能非常好”等带有倾向性的诱导"
  },
  {
    "title": "添加防糊弄反向校验题",
    "advice": "在量表中间穿插 1 道简单的反向验证题（如“本题请选择非常同意”），用于筛选胡乱勾选的作废问卷",
    "avoid": "严禁忽略垃圾数据导致分析结论偏差"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[WENJUAN_SCORES\][\s\S]*?\[\/WENJUAN_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[WENJUAN_SCORES\](.*?)\[\/WENJUAN_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('wenjuan_history_records');
...
const saveHistory = () => {
  localStorage.setItem('wenjuan_history_records', JSON.stringify(historyList.value));
};
